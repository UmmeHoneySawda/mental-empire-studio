  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {jobs.map((job) => {
        ...
        return (
          <div key={job.id} className="automation-conveyor-lane">
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div role="progressbar" aria-valuenow={job.progress} aria-valuemin={0} aria-valuemax={100} style={{ height: 6, borderRadius: 999, background: 'var(--border-2)', overflow: 'hidden', marginTop: 8 }}>
                <div
                  style={{
                    height: '100%',
                    width: `${job.progress}%`,
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 8, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
                <span style={{ fontSize: 11, color: 'var(--text-dim)', fontFamily: 'var(--font-mono)' }}>
                  {job.currentStep || '—'} · {job.completedCount}/{Math.max(1, job.totalItems)} items{job.warningCount ? ` · ${job.warningCount} warnings` : ''}
                  {job.failedCount ? ` · ${job.failedCount} failed` : ''}
                </span>
                <span style={{ fontSize: 11, color: 'var(--text-faint)' }}>{new Date(job.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
              </div>

              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10, alignItems: 'center' }}>
                <Btn size="sm" variant="soft" onClick={() => onExpand(job)}>
                  {isExpanded ? 'Hide details' : 'View details'}
                </Btn>
                <div style={{ flex: 1 }} />
                {(job.status === 'running' || job.status === 'queued') && (
                  <Btn size="sm" variant="ghost" onClick={() => onPause(job.id)}>
                    Pause
                  </Btn>
                )}
                {(job.status === 'paused' || job.status === 'failed' || job.status === 'attention') && (
                  <Btn size="sm" variant="ghost" onClick={() => onResume(job.id)}>
                    Resume
                  </Btn>
                )}
                {(job.status === 'failed' || job.status === 'attention') && (
                  <Btn size="sm" variant="ghost" onClick={() => onRetry(job.id)}>
                    Retry failed
                  </Btn>
                )}
                {['queued', 'running', 'pausing', 'paused', 'failed', 'attention'].includes(job.status) && (
                  <Btn size="sm" variant="ghost" onClick={() => onCancel(job.id)}>
                    Cancel
                  </Btn>
                )}
                <IconBtn title="Delete run" danger onClick={() => onDelete(job)}>
                  ✕
                </IconBtn>
              </div>
                ...
                <div style={{ marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                    ...
                    {expanded.steps.map((step) => (
                      <div key={step.id} style={{ border: '1px solid var(--border)', borderRadius: 9, padding: 9, background: 'var(--bg-inset)' }}>
                        <div style={{ display: 'flex', gap: 7, alignItems: 'center' }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 600 }}>{step.label}</span>
                        </div>
                        <div role="progressbar" aria-valuenow={step.progress} style={{ marginTop: 6, height: 4, borderRadius: 3, background: 'var(--border-2)' }}>
                          <div style={{ height: '100%', width: `${step.progress}%`, background: step.status === 'failed' ? 'var(--err)' : 'var(--accent)', borderRadius: 3 }} />
                        </div>
                        {step.error && <div style={{ color: 'var(--err-2)', fontSize: 11, marginTop: 6 }}>{step.error}</div>}
                      </div>
                    ))}
                  </div>
                      ...
                      {expanded.items.map((item) => (
                        ...
                        >
                          ...
                          {item.projectId && onOpenProject && (
                            <Btn size="sm" variant="ghost" onClick={() => onOpenProject(item.projectId!)}>
                              Open
                            </Btn>
                          )}
                        </div>
                  ...
                  >
                    {expanded.logs.length === 0
                      ? 'No log entries yet.'
                      : expanded.logs.slice(-20).map((row) => (
                          <div key={row.id}>
                            {new Date(row.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} · {row.message}
                          </div>
                        ))}
                  </div>
                </div>
