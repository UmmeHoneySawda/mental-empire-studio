...
/* Drafts and builders for the Cinematic set.
 *
 ...
 * the matching prop, so the existing Beats list edits the same line this module builds.
 *
 * This lives in shared/ rather than beside the accordion because the Electron main process builds
 * the same plan for an unattended automation batch, and main cannot import from src/. The renderer
 * keeps importing it through src/features/video-studio/editor/newTemplates.ts, which is now a
 * re-export barrel. */
