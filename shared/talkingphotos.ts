...
export function planSplit(input: TpPlanInput): TpPlan {
  ...
  if (!Number.isFinite(partSeconds) || partSeconds <= 0) {
    return { outputs: [], totalParts: 0, totalOutputs: 0, coveredSec: 0, droppedTailSec: 0, warnings: ['Chunk length must be a positive number of seconds.'] }
  }
  if (partSeconds > mergeCapSec) {
    return { outputs: [], totalParts: 0, totalOutputs: 0, coveredSec: 0, droppedTailSec: 0, warnings: [`Chunk length cannot exceed the ${mergeCapSec}s merge cap.`] }
  }
  if (sourceDurationSec < minPartSeconds) {
    return { outputs: [], totalParts: 0, totalOutputs: 0, coveredSec: 0, droppedTailSec: 0, warnings: ['The source audio is too short to render.'] }
  }

  const partsPerOutput = Math.max(1, Math.floor(mergeCapSec / partSeconds))
  const outputSpan = partsPerOutput * partSeconds
  ...
  while (cursor < sourceDurationSec) {
    ...
    while (partCursor < outputEnd) {
      const partEnd = Math.min(outputEnd, partCursor + partSeconds)
