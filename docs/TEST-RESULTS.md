...
| P9 | `electron/main.ts`, `electron/services/render.ts`, `electron/services/broll.ts` | The old long-form slowdown could regress if image/B-roll renders accidentally reintroduced `zoompan`, large `xfade` graphs, or direct looped B-roll inputs. | M6 now asserts long single-image and 3-image renders contain no `zoompan` or `xfade`, long multi-image renders use concat, long B-roll renders use the concat-manifest input, no direct `-stream_loop`, no `xfade`, and still pin `-t 1174.00`. | ✅ fixed |
...
- `SMOKE_M6_LONGFORM captions=true wordEvents=1600 phraseEvents=200 motion=true brollFast=true`.
