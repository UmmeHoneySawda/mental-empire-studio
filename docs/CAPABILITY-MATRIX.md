...
| NVIDIA GPU with a passing NVENC encode probe | NVENC | Uses `h264_nvenc -preset p5 -tune hq -rc vbr -cq ... -b:v 0`. |
| Intel Quick Sync with a passing QSV encode probe | QSV | Uses `h264_qsv` with global quality. |
| AMD AMF with a passing AMF encode probe | AMF | Uses `h264_amf` CQP quality mode. |
